...
| 011 | Cut khoét ground để **viền/góc thừa** + **"pie-slice wedge"** tròn/ellipse + cut==zone không xóa. Root: earcut-hole chạm biên degenerate + cover-test ray-cast nhập nhằng ở biên. Fix: **polygon-boolean difference** (`polygon-clipping`/Martinez) thay clip-hole. Gotcha: lib ESM chỉ export default (`.d.ts` named lệch runtime) → verify export bằng **esbuild bundle** | ground | geometry | high | fixed | 2026-06-07 |
| 012 | ≥2 hồ phản chiếu → gương ĐƠ mọi góc (1 hồ ok). Root: three reflector cờ module-global `_inReflector` + `virtualCamera = camera.clone()` COPY layers → 2 hồ render-lẫn-nhau. Fix: mặt nước layer riêng + `excludeReflectionLayer` disable layer trên virtualCamera MỖI frame | pond | gpu-dispose | high | fixed | 2026-06-07 |
| 013 | THREE.Points trên **WebGPU**: point LUÔN 1px + `pointUV` WebGL-only (emit GLSL thô) + PointsNodeMaterial offset clip-space theo `positionGeometry.xy`×size → spawn-pos ≠ 0 = **hạt văng phủ kín màn hình (màn mờ xanh)**. Fix: particle WebGPU = **InstancedMesh quad + SpriteNodeMaterial** (positionNode = instancedBufferAttribute) + frustumCulled=false | thac-nuoc | shader | high | fixed | 2026-06-10 |
| 014 | `viewportSharedTexture` = 1 FramebufferTexture **MODULE-GLOBAL** (`let _sharedFramebuffer`) — 2 renderer cùng page (editor + Lab preview) giành resize/copy → flood `copy range touches outside` + command buffer invalid + dispose nổ `undefined.destroy` → rebuild fail. Fix: module chạy được ở renderer-thứ-2 dùng **`viewportTexture` per-instance** + TỰ giữ ref texture để dispose (né leak). Cùng họ KI-012 (three module-global state) | thac-nuoc, pond | shader | high | fixed | 2026-06-10 |
