...
- **Lighting pattern (đèn fixture MỚI):** `site/lighting/` (lõi `SiteLightingSystem`) + archplan `lighting/` (controller/panel/drag) — **tách riêng**, persist `archplan:lighting`. F1 đèn pha uplight ✅ (no-shadow, +0 sampler). Thêm fixture (F2–F6) **cắm vào lõi, KHÔNG phình god-module**. Pool point cũ gộp vào Phase 3.
