...
| Khung CHỮ NHẬT chớp lòe lốm đốm sát má/đáy (vân tường chen màu khung) | vành trong khung kết thúc ĐÚNG mép lỗ → mặt trong ĐỒNG PHẲNG reveal tường (cùng x/y, dải z giao) = z-fight | rectFrame +`lip` (FRAME_LIP kẹp lỗ nhỏ /6) — vành trong lấn vào lòng lỗ che reveal; mặt ngoài giữ nguyên. Ghi chú cũ "rect không cần lip" SAI (2026-06-11, ảnh #5) |

## 5. Lịch sử nâng cấp

- `2026-06-11` — **FIX khung chữ nhật CHỚP LÒE vành trong (ảnh #5, sau C3)**: má/đầu/bậu lấn `lip` vào lòng lỗ (FRAME_LIP, kẹp lỗ nhỏ /6) — mặt trong khung hết đồng phẳng với reveal tường, lip che hẳn mặt reveal; mặt ngoài khung giữ nguyên vị trí. Bonus: khung đè mép cánh ~25mm = door-stop như cửa thật.
- `2026-06-11` — **C3 SONG SẮT cửa sổ (joinery phase 3 — chốt trọn kế hoạch C1→C4)**: `barStyle` ('vert' thanh dọc / 'grid' + thanh ngang dẹt) + `barColor` (optional parse-tolerant, chỉ kind window). `barsGeosLocal` (Joinery): thanh dọc cylinder r8mm openEnded bước ≤13cm (chuẩn an toàn song sắt thật) cắm suốt lỗ; **lỗ TRÒN = từng thanh cắt theo CHORD ellipse tại x** (`barSpanAt` — khớp lỗ carve fit-bbox), strap ngang chord theo y. Bake bucket `frame:color` cùng khung (0 draw mới nếu trùng màu); static — không cần live. GUI: dropdown Song sắt + Màu song (chỉ window).
