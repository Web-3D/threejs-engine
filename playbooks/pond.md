...
| **2+ hồ** đang bật → gương 1 hồ ĐƠ mọi góc (+ "không ăn bóng" = ảnh đơ cũ) | three: reflector dùng chung cờ module `_inReflector`+temp (`bounces:false`) — hồ A render RTT lại render mặt nước hồ B → chen nested-render | **FIXED** `known-issues/KI-012` — mặt nước → **layer riêng** + `WaterSurface.excludeReflectionLayer` **disable layer đó trên virtualCamera mỗi frame** (vì `getVirtualCamera`=`camera.clone()` copy layers!). Camera+raycaster enable layer (vẫn thấy/click). KHÔNG patch three; mất "nước-soi-nước" (vô hại). |
...
- `2026-06-10` — **ĐÁ VIỀN DẸT-BÈ** (NgQuan: "dẹt ngang, hạ thấp độ random chiều cao, bè bè bớt nguy hiểm"): `stoneAt` scale ngang 0.78..1.18→**0.95..1.35** (bè ra), cao 0.7..1.15→**0.42..0.56** (dẹt + range hẹp = ít random chiều cao), tâm chìm 0.4r→0.3r (ngồi sát coping). Scale Y áp SAU jag → góc cạnh cũng bẹp theo. Kèm: trả nợ eslint `stoneWallEdge`/`buildSiteFence` (3d8d82f) — tách verts/indices + geos/material, 0 đổi hành vi.
- `2026-06-10` — **ĐÁ VIỀN khép kín qua op #1 arcLength** (nâng cấp "chờ ops/" — Factory commit kệ sáng nay): `pondStoneGeos` bỏ walk tay segment-by-segment → polyline khép kín thành curve fn + `arcLength` (ops/resample, samples ≥ max(128, 2n)); sinh HẾT bước adaptive trước rồi **chia lại khít chu vi (k = L/Σstep)** → vòng đá KHÉP KÍN không mối nối (walk cũ dừng đâu hở đó — seam last→first tùy chu vi); bước cuối dùng bán kính viên 0 (neighbor wrap). Số viên m = round(L/0.82d). **`copyToPoints` KHÔNG dùng** (điều chỉnh ghi sổ cũ): mỗi viên có jag geometry RIÊNG đã merge = 1 draw sẵn — instancing ép share geometry (mất jag per-viên), không lợi draw call.
- `2026-06-10` — **ĐÁ VIỀN to-nhỏ xen kẽ + góc cạnh** (NgQuan: "to hơi đều… thêm random… min max từ tâm ra các cạnh, đừng tròn quá"): state +`borderStoneVar`/`borderStoneJag` (% 0–100, default 45/35, parse tolerant — áp CẢ save cũ theo chủ ý). Render: bán kính mỗi viên ×[1±0.45v] (`fAt` hash idx) + **bước xếp tự giãn theo (r_i+r_{i+1})×0.82** (viên to chiếm chỗ rộng, vẫn liền); góc cạnh = `jagVertices` — đỉnh icosa lệch bán kính [1±0.45j] từ tâm, **sin-hash theo (VỊ TRÍ, idx)** → vertex trùng vị-trí lệch giống nhau, icosa non-indexed KHÔNG nứt mặt (bài KI-003) + `computeVertexNormals` giữ faceted. GUI +2 slider "Đá to-nhỏ %"/"Đá góc cạnh %" (chỉ shape cong; commit-only). ⚠️ eslint lộ 2 lỗi PRE-EXISTING `stoneWallEdge`/`buildSiteFence` (commit 3d8d82f 06-05, ngoài vùng sửa — chưa xử).
